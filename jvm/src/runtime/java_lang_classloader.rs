...
use crate::{class_instance::ClassInstance, jvm::Jvm, JvmResult};
...
impl JavaLangClassLoader {
    pub async fn get_system_class_loader(jvm: &Jvm) -> JvmResult<Box<dyn ClassInstance>> {
    ...
    pub async fn load_class(jvm: &Jvm, this: Box<dyn ClassInstance>, class_name: &str) -> JvmResult<Option<Box<dyn ClassInstance>>> {
