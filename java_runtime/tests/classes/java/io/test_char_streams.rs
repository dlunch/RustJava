...
async fn cr_01_cr_02_string_reader_contract() -> Result<()> {
    ...
    let _: () = jvm.invoke_virtual(&reader, "mark", "(I)V", (10,)).await?;
    ...
    assert_eq!(jvm.invoke_virtual::<_, i32>(&reader, "read", "()I", ()).await?, '한' as i32);
    assert_eq!(jvm.invoke_virtual::<_, i64>(&reader, "skip", "(J)J", (-2i64,)).await?, 0);
    assert_eq!(jvm.invoke_virtual::<_, i64>(&reader, "skip", "(J)J", (10i64,)).await?, 2);
    ...
    let invalid_mark: Result<()> = jvm.invoke_virtual(&reader, "mark", "(I)V", (-1,)).await;
    ...
    let _: () = jvm.invoke_virtual(&reader, "close", "()V", ()).await?;
    let closed: Result<bool> = jvm.invoke_virtual(&reader, "ready", "()Z", ()).await;
    ...
    let closed: Result<i32> = jvm.invoke_virtual(&reader, "read", "()I", ()).await;
    let Err(JavaError::JavaException(exception)) = closed else {
        panic!("read after close must throw IOException");
    };
    assert!(jvm.is_instance(&*exception, "java/io/IOException"));
    let null_chars: ClassInstanceRef<Array<JavaChar>> = None.into();
    let closed: Result<i32> = jvm.invoke_virtual(&reader, "read", "([CII)I", (null_chars, -1, 1)).await;
    let Err(JavaError::JavaException(exception)) = closed else {
    ...
    let closed: Result<i32> = jvm.invoke_virtual(&reader, "read", "([CII)I", (target, 2, 1)).await;
    let Err(JavaError::JavaException(exception)) = closed else {
...
async fn cr_03_cr_04_char_array_reader_contract() -> Result<()> {
    ...
    assert_eq!(jvm.invoke_virtual::<_, i32>(&default_reader, "read", "()I", ()).await?, 'X' as i32);
    ...
    assert_eq!(jvm.invoke_virtual::<_, i64>(&reader, "skip", "(J)J", (1i64,)).await?, 1);
    let _: () = jvm.invoke_virtual(&reader, "reset", "()V", ()).await?;
    assert_eq!(jvm.invoke_virtual::<_, i32>(&reader, "read", "()I", ()).await?, 'B' as i32);

    let target = jvm.instantiate_array("C", 2).await?;
    assert_eq!(jvm.invoke_virtual::<_, i32>(&reader, "read", "([CII)I", (target.clone(), 0, 1)).await?, 1);
    assert_eq!(jvm.load_array::<JavaChar>(&target, 0, 2).await?, ['C' as JavaChar, 0]);
    assert_eq!(jvm.invoke_virtual::<_, i32>(&reader, "read", "()I", ()).await?, -1);
    assert_eq!(jvm.invoke_virtual::<_, i32>(&reader, "read", "([CII)I", (target.clone(), 0, 0)).await?, 0);
    assert!(!jvm.invoke_virtual::<_, bool>(&reader, "ready", "()Z", ()).await?);
    assert_eq!(jvm.invoke_virtual::<_, i64>(&reader, "skip", "(J)J", (-1i64,)).await?, 0);

    let invalid: Result<i32> = jvm.invoke_virtual(&reader, "read", "([CII)I", (target, 2, 1)).await;
    let Err(JavaError::JavaException(exception)) = invalid else {
...
async fn cw_01_cw_02_char_array_writer_contract() -> Result<()> {
    ...
    let _: () = jvm.invoke_virtual(&writer, "write", "(Ljava/lang/String;II)V", (value, 1, 2)).await?;
    assert_eq!(jvm.invoke_virtual::<_, i32>(&writer, "size", "()I", ()).await?, 5);
    ...
    let copy: ClassInstanceRef<Array<JavaChar>> = jvm.invoke_virtual(&writer, "toCharArray", "()[C", ()).await?;
    assert_eq!(
    ...
    let string: ClassInstanceRef<String> = jvm.invoke_virtual(&writer, "toString", "()Ljava/lang/String;", ()).await?;
    assert_eq!(JavaLangString::to_rust_string(&jvm, &string).await?, "ABCDE");
    let output = jvm.new_class("java/io/StringWriter", "()V", ()).await?;
    let _: () = jvm.invoke_virtual(&writer, "writeTo", "(Ljava/io/Writer;)V", (output.clone(),)).await?;
    let string: ClassInstanceRef<String> = jvm.invoke_virtual(&output, "toString", "()Ljava/lang/String;", ()).await?;
    assert_eq!(JavaLangString::to_rust_string(&jvm, &string).await?, "ABCDE");

    let _: () = jvm.invoke_virtual(&writer, "flush", "()V", ()).await?;
    let _: () = jvm.invoke_virtual(&writer, "close", "()V", ()).await?;
    let _: () = jvm.invoke_virtual(&writer, "write", "(I)V", ('F' as i32,)).await?;
    assert_eq!(jvm.invoke_virtual::<_, i32>(&writer, "size", "()I", ()).await?, 6);
    let _: () = jvm.invoke_virtual(&writer, "reset", "()V", ()).await?;
    assert_eq!(jvm.invoke_virtual::<_, i32>(&writer, "size", "()I", ()).await?, 0);

    let default_writer = jvm.new_class("java/io/CharArrayWriter", "()V", ()).await?;
    assert_eq!(jvm.invoke_virtual::<_, i32>(&default_writer, "size", "()I", ()).await?, 0);
