...
impl Interpreter {
    pub async fn run(jvm: &Jvm, code_attribute: &AttributeInfoCode, args: Box<[JavaValue]>, return_type: &JavaType) -> Result<JavaValue> {
        let mut stack_frame = StackFrame::new(code_attribute.max_locals as usize, code_attribute.max_stack as usize);
        let mut local_index = 0;
        for value in args {
            let value = Self::to_stack_frame_type(value);
            // long and double take two slots in local variables.
            let slots = if matches!(value, JavaValue::Long(_) | JavaValue::Double(_)) {
                2
            } else {
                1
            };
            stack_frame.local_variables[local_index] = value;
            local_index += slots;
        }
        ...
        loop {
            ...
            match result {
                ...
                Err(JavaError::JavaException(e)) => {
                    let exception_handler = Self::find_exception_handler(jvm, &*e, code_attribute, pc).await;
                ...
                Opcode::Areturn | Opcode::Dreturn | Opcode::Freturn | Opcode::Ireturn | Opcode::Lreturn => {
                    ...
                    if matches!(opcode, Opcode::Ireturn) {
                        ...
                        if *return_type == JavaType::Boolean {
                            return Ok(JavaValue::Boolean(value & 1 != 0));
