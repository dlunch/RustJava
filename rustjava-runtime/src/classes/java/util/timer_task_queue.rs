...
impl TimerTaskQueue {
    ...
    async fn init(jvm: &Jvm, _: &mut RuntimeContext, mut this: ClassInstanceRef<Self>) -> Result<()> {
        ...
        jvm.put_field(&mut this, "java/util/Timer$TaskQueue", "queue", "[Ljava/util/TimerTask;", queue)
            .await?;
        Ok(())
    }

    pub(crate) async fn add(jvm: &Jvm, this: &mut ClassInstanceRef<Self>, task: ClassInstanceRef<TimerTask>) -> Result<bool> {
        let size: i32 = jvm.get_field(this, "java/util/Timer$TaskQueue", "size", "I").await?;
        let mut queue: ClassInstanceRef<Array<TimerTask>> = jvm
            .get_field(this, "java/util/Timer$TaskQueue", "queue", "[Ljava/util/TimerTask;")
            .await?;
        if size as usize + 1 == jvm.array_length(&queue).await? {
            ...
            queue = expanded;
            jvm.put_field(this, "java/util/Timer$TaskQueue", "queue", "[Ljava/util/TimerTask;", queue.clone())
                .await?;
        }
        ...
        while child > 1 {
            ...
            let parent_time: i64 = jvm.get_field(&parent_task, "java/util/TimerTask", "nextExecutionTime", "J").await?;
            let task_time: i64 = jvm.get_field(&task, "java/util/TimerTask", "nextExecutionTime", "J").await?;
        ...
        jvm.put_field(this, "java/util/Timer$TaskQueue", "size", "I", size + 1).await?;
        Ok(child == 1)
    }
    ...
    pub(crate) async fn remove_min(jvm: &Jvm, this: &mut ClassInstanceRef<Self>) -> Result<()> {
        ...
        let new_size = size - 1;
        jvm.put_field(this, "java/util/Timer$TaskQueue", "size", "I", new_size).await?;
        if new_size == 0 {
            return Ok(());
        }

        let last_time: i64 = jvm.get_field(&last, "java/util/TimerTask", "nextExecutionTime", "J").await?;
        ...
        while parent * 2 <= new_size as usize {
            ...
            let mut child_time: i64 = jvm.get_field(&child_task, "java/util/TimerTask", "nextExecutionTime", "J").await?;
            if child < new_size as usize {
                let right: ClassInstanceRef<TimerTask> = jvm.load_array(&queue, child + 1, 1).await?.remove(0);
                let right_time: i64 = jvm.get_field(&right, "java/util/TimerTask", "nextExecutionTime", "J").await?;
    ...
    pub(crate) async fn reschedule_min(jvm: &Jvm, this: &mut ClassInstanceRef<Self>, new_time: i64) -> Result<()> {
        ...
        jvm.put_field(&mut task, "java/util/TimerTask", "nextExecutionTime", "J", new_time)
            .await?;
    ...
    pub(crate) async fn clear(jvm: &Jvm, this: &mut ClassInstanceRef<Self>) -> Result<()> {
        ...
        if size > 0 {
            ...
            jvm.put_field(this, "java/util/Timer$TaskQueue", "size", "I", 0i32).await?;
        }
        Ok(())
    }
}
